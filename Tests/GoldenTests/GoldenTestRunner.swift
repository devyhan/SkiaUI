...
public struct GoldenTestRunner {
    ...
    private func describeCommand(_ cmd: DrawCommand) -> String {
        ...
        case .drawText(let text, let x, let y, let fs, let fw, let c, let bw, let ff, let ll, let lbm):
            let familyStr = ff.map { ", ff=\($0)" } ?? ""
            let llStr = ll.map { ", ll=\($0)" } ?? ""
            let lbmStr = lbm != 0 ? ", lbm=\(lbm)" : ""
            return "drawText(\"\(text)\", \(x), \(y), fs=\(fs), fw=\(fw), 0x\(String(c, radix: 16, uppercase: true)), bw=\(bw)\(familyStr)\(llStr)\(lbmStr))"
        case .drawImage(let src, let x, let y, let w, let h, let cm): return "drawImage(\"\(src)\", \(x), \(y), \(w), \(h), cm=\(cm))"
